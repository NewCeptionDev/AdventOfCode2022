...
const distanceToValve = (current: string, target: string, wayMap: Map<string, Map<string, string[]>>): number => {
...
const buildWayMap = (valves: Valve[]): Map<string, Map<string, string[]>> => {
  ...
  while (Array.from(wayMap.keys()).some(key => wayMap.get(key).size < valves.length)) {
    Array.from(wayMap.keys()).filter(key => wayMap.get(key).size < valves.length).forEach(key => {
      const mostSteps = Array.from(wayMap.get(key).values()).map(val => val.length).sort((a, b) => a - b).pop()
      const furthestKnown = Array.from(wayMap.get(key).keys()).filter(lowerKey => wayMap.get(key).get(lowerKey).length === mostSteps)
