...
import { readInputFromSpecialFile, readTestFile, splitToAllLines, splitToLines } from "../utils/readInput"
...
function splitToLargeAndSmallCoords(x: number, y: number, M: number) {
  const lx = Math.floor(x / M); const ly = Math.floor(y / M);
  const sx = mod(x, M); const sy = mod(y, M);
  return [lx, ly, sx, sy];
}
